...
export class RestraFind {
  ...
  // tabBarElement: any;
  items: any;
  public resultRestra: any;
  public resultRestraLength : any;
  constructor(public navCtrl: NavController, public loadingCtrl: LoadingController, public restraList: Providers, public platform: Platform) {
    ...
    this.resultRestraLength = this.restraList.items.length;
  ...
  public getItems(ev: any) {
    ...
    this.restraList.restraListItems();
    this.resultRestra = this.restraList.items;

    // set val to the value of the ev target
    var val = ev.target.value;
    console.log(val);
    if (val == "") {
      this.resultRestraLength = this.resultRestra.length;
      this.restraList.items = this.resultRestra;
    }
    else {
      this.items = this.resultRestra.filter((item) => {
        return (item.name.toLowerCase().indexOf(val.toLowerCase()) > -1);
      })
      this.resultRestraLength = this.items.length;
      this.restraList.items = this.items;
    }
